...
fun String.toMoneyValue(): String {

    val amount = this.toLongOrNull() ?: return "N/A"

    if (amount <= 0L) return "N/A"

    return when {
        amount >= 1_000_000_000L -> {
            "$${formatCompactNumber(amount / 1_000_000_000.0)}B"
        }

        amount >= 1_000_000L -> {
            "$${formatCompactNumber(amount / 1_000_000.0)}M"
        }

        amount >= 1_000L -> {
            "$${formatCompactNumber(amount / 1_000.0)}K"
        }
